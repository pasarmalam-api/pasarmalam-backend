...
(async()=>{
 ...
 const navigation=new Map();const inventory=[];
 for(const file of fs.readdirSync(root).filter(f=>f.endsWith('.html'))){
   ...
   inventory.push({page:file,commands:await page.locator('button').allTextContents()});
   for(const href of await page.locator('a[href]').evaluateAll(a=>a.map(x=>x.getAttribute('href')))){
     if(/^[a-z-]+\.html(?:\?.*)?$/.test(href)){
       assert(fs.existsSync(path.join(root,href.split('?')[0])),file+' broken link '+href);
       if(!navigation.has(href))navigation.set(href,file);
     }
   }
 ...
 for(const file of ['orders.html','returns.html','reviews.html','support.html','wallet.html','performance.html']){
   await visit(file);await page.getByRole('button',{name:'Refresh',exact:true}).click();
 }
 await visit('index.html');await page.locator('.seller-lang-toggle').click();assert.equal(await page.evaluate(()=>localStorage.getItem('pasarmalam-lang')),'zh');
 await page.locator('.seller-lang-toggle').click();await page.locator('.seller-lang-toggle').click();
 await page.setViewportSize({width:402,height:900});await page.locator('.seller-menu-toggle').click();assert.equal(await page.locator('.seller-menu-toggle').getAttribute('aria-expanded'),'true');await page.keyboard.press('Escape');assert.equal(await page.locator('.seller-menu-toggle').getAttribute('aria-expanded'),'false');await page.setViewportSize({width:1440,height:1000});
 for(const [href,file] of navigation){
   await visit(file);
   const target=page.locator('a[href="'+href+'"]').filter({visible:true}).first();
   if(await target.count()){await target.click();await page.waitForURL(origin+'/'+href);}
 }
 fs.mkdirSync('../outputs/seller-audit',{recursive:true});
 fs.writeFileSync('../outputs/seller-audit/command-inventory.json',JSON.stringify(inventory,null,2));
 await visit('products.html');await page.locator('#q').fill('no matching product');assert.equal(await page.locator('#rows tr').count(),0);await page.locator('#q').fill('Camera');assert.equal(await page.locator('#rows tr').count(),1);
 await page.locator('#sellerBack').click();await page.waitForURL(origin+'/index.html');
 ...
 for(const bucket of ['unpaid','to_pack','shipped','delivered','completed','cancelled']){await page.locator(`button[onclick="current='${bucket}';render()"]`).click();}
 ...
 await context.addInitScript(()=>{localStorage.removeItem('pm_token');localStorage.removeItem('pm_user')});
 await visit('login.html?next=order-detail.html%3Fid%3D201');
 await fill({email:'seller@example.test',password:'test-password'});
 await page.locator('#viewBtn').click();assert.equal(await page.locator('#password').getAttribute('type'),'text');await page.locator('#viewBtn').click();
 fail=true;await page.locator('button[onclick="login()"]').click();await page.waitForTimeout(200);assert.match(await page.locator('#status').textContent(),/Simulated validation failure/);
 fail=false;await page.locator('button[onclick="login()"]').click();await page.waitForURL('**/order-detail.html?id=201');
 await visit('register.html');
 await fill({sellerName:'Test Seller',sellerPhone:'0110000000',sellerEmail:'seller@example.test',sellerPassword:'test-password',shopName:'Test Shop',identityNumber:'TEST123',sellerAddress:'Test address',bankName:'Test Bank',bankAccountName:'Test Seller',bankAccountNumber:'123456'});
 await page.locator('#identityType').selectOption({index:1});await page.locator('#shopCategory').selectOption('Chargers');
 await page.locator('button[onclick="sendSellerOtp()"]').click();await page.waitForTimeout(150);assert.equal(writes.at(-1).endpoint,'/api/otp/email/send');
 await page.locator('#otp').fill('123456');await page.locator('button[onclick="verifySellerOtp()"]').click();await page.waitForTimeout(150);assert.equal(writes.at(-1).endpoint,'/api/otp/email/verify');
 await page.locator('button[onclick="applySeller()"]').click();await page.waitForURL('**/thank-you.html');assert.equal(writes.at(-1).endpoint,'/api/auth/signup');
 assert.deepEqual(errors,[]);
 console.log('PASS seller populated workflow suite, navigation, refresh, menu, language, sign-in and OTP registration');
