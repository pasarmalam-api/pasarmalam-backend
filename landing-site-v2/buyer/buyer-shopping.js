 ...
 if(page==='cart.html'){
  ...
  draw=function(){
   ...
   cartList.innerHTML=cart.length?cart.map(i=>{
    ...
    return `<div class="line"><div class="checkout-choice"><input type="radio" name="checkoutItem" aria-label="Select ${esc(i.name)}" value="${esc(i.id)}" ${String(selected)===String(i.id)?'checked':''} ${available?'':'disabled'}><div class="cart-product">${image?`<img src="${esc(image)}" alt="${esc(i.name)}">`:'<span></span>'}<div><a href="product.html?id=${encodeURIComponent(i.product_id)}">${esc(i.name)}</a><p class="muted">${esc(i.shop)}${i.variant?' | '+esc(i.variant):''}</p><p class="price">${money(i.price)} <span class="muted">&times; ${Number(i.quantity)}</span></p><span class="muted">${available?'Stock: '+Number(i.stock):'Out of stock'}</span></div></div></div><div><div class="qty"><button class="step" aria-label="Decrease quantity" onclick="changeQty(${Number(i.id)},${Number(i.quantity)-1})" ${!available||i.quantity<=1?'disabled':''}>-</button><b>${Number(i.quantity)}</b><button class="step" aria-label="Increase quantity" onclick="changeQty(${Number(i.id)},${Number(i.quantity)+1})" ${!available||i.quantity>=i.stock?'disabled':''}>+</button></div><button class="danger" onclick="removeItem(${Number(i.id)})">Remove</button></div></div>`;
   ...
   checkoutBtn.disabled=!item||busy;checkoutBtn.textContent='Checkout selected item';
  };
  cartList.addEventListener('change',e=>{if(e.target.name==='checkoutItem'){selected=e.target.value;draw()}});
  goCheckout=function(){const item=cart.find(i=>String(i.id)===String(selected));if(!item||!token())return;location.href='checkout.html?'+new URLSearchParams({product_id:item.product_id,quantity:item.quantity,variant:item.variant||''})};
